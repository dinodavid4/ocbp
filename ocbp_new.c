...
int calculatePriority(struct rtmixcrit_vcpu * vcpu_to_schedule) 
{
	...
	for(j_fxd_index = 0; j_fxd_index < NO_OF_JOBS ; j_fxd_index++) {
		for(vcpu_index = 0; vcpu_index < (NO_OF_JOBS-loop_count) ; vcpu_index++) {
			...
			if(vcpu_to_schedule[vcpu_index].criticality_vcpu == HIGH_CRITICALITY) {
				...
				for(count = vcpu_index + 1; count < (NO_OF_JOBS-loop_count) ; count++) {
		...
		if(vcpu_index == (NO_OF_JOBS -1 -loop_count)) {
			/* No need to shift the arrays because of the last element*/
		}
		else {
			for(count = vcpu_index; count < (NO_OF_JOBS-1-loop_count); count++) {
...
int calculateLCM(struct rtmixcrit_vcpu * vcpu_to_schedule, int no_of_vcpu)
{
...
{
	int loop_count = 0;
	struct rtmixcrit_vcpu vcpu_to_schedule[NO_OF_JOBS]; 	

	struct rtmixcrit_vcpu vcpu_prioritized[NO_OF_JOBS]; 	
