 ...
 *
 * Priority scheme: OCBP, Own Criticality based Priority.
 * At any scheduling point, the VCPU with highest priority is scheduled.
 * Priority assignment is done offline
 *
...
void findInstance(struct rt_vcpu * vcpu_to_schedule, int no_of_vcpu)
{
	...
	//struct rt_vcpu * final_vcpu_to_schedule;
...
__runq_insert(const struct scheduler *ops, struct rt_vcpu *svc)
{
	...
	static struct rt_vcpu vcpu_prioritized[3];
	//int loop = 0;
		...
		struct xen_domctl_scheduler_op *op)
{
	...
	static struct rt_vcpu local_vcpu[3];
			...
			list_for_each( iter, &sdom->vcpu ) 
			{
				...
				{ 
					...
					if ( vcpu_index < 0 ) 
						...
						local_vcpu[loop_index].budget_low = op->u.rt.budget_low;
						local_vcpu[loop_index].budget_high = op->u.rt.budget_high;
						local_vcpu[loop_index].deadline = op->u.rt.deadline;
						local_vcpu[loop_index].criticality_vcpu = op->u.rt.criticality_vcpu;
					...
					if ( vcpu_index >= 0 ) 
					{
						...
						if(loop_index == 2)
						{
							...
							while(first_vcpu != NULL)
							{
								//__runq_insert(ops, &(first_vcpu->vcpu_data));
								vcpu_prioritized[loop_count] = first_vcpu->vcpu_data;
								printk("OCBP:_Cal: Deadline Time = %ld Deadline Time from Prioritized VCPU %ld\n",
										first_vcpu->vcpu_data.deadline, vcpu_prioritized[loop_count].deadline);
								first_vcpu = first_vcpu->next_vcpu;
								loop_count++;
							}
						...
						{
							printk("OCBP:System in High Criticality mode");
